...
<div class="bg-white p-6 rounded-lg shadow-lg w-96 text-center flex w-[600px] wT-[900px]">
    ...
    <div class="w-1/2 p-4">
    <form id="auth-form" class="space-y-4 mt-6" action="{{route('traitement')}}" method="POST">
    @csrf
        ...
        <div class="text-left" id="emailgroup">
        ...
        <div id="confirm-password-group" class="text-left relative">
            <label for="confirm-password" class="block font-medium">Confirmer le mot de passe <span class="text-red-500">*</span></label>
            <input type="password" id="confirm-password" class="w-full p-2 border rounded pr-10">
        </div>
        <ul>
            @foreach ($errors as $error)
                <li class="text-blue bg-red">{{$error}}</li>
            @endforeach
        </ul>
    ...
    <div class="flex justify-between mt-4">
        <button id="sign-in-btn" class="w-1/2 p-2 bg-[#501F3A] text-white rounded-l active hover:bg-[#CCCCCC] transition" onclick="window.location.href='/login';">Sign In</button>
        <button id="sign-up-btn" class="w-1/2 p-2 bg-gray-300 text-black rounded-r hover:bg-[#0F292F] transition hover:text-white">Sign Up</button>
  </div>
</div>
</div>
@endsection
